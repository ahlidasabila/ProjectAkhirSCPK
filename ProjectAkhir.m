...
opts.SelectedVariableNames = [2,3,4,5,6,7,8];
dataKeputusan = readtable('nilai.csv', opts);
...
opts2.SelectedVariableNames = 'SMA';
...
elseif(indonesia == ' ' || inggris == ' ' || matematika == ' ' || fisika == ' ' || kimia == ' '|| biologi == ' ')
    ...
    hasilakhir = "Perangkingan SMA Negeri di Jawa Timur terbaik adalah " + temp(1) + " berdasarkan rerata nilai UNBK.";
...
inputindonesia = get(handles.indonesia,'String');
...
if(isempty(inputindonesia) || isempty(inputinggris) || isempty(inputmatematika) || isempty(inputfisika) || isempty(inputkimia) || isempty(inputbiologi))
    ...
    indonesia = str2double(inputindonesia);
